...
QVariantList KeyboardGlobal::getKbLayouts()
{
    QVariantList kbLayouts;

    KeyboardGlobal::LayoutsMap layouts = getKeyboardLayouts();
    QList< QPair<QString, KeyboardGlobal::KeyboardInfo> > kbLayoutsList;

    for (KeyboardGlobal::LayoutsMap::const_iterator i = layouts.constBegin(); i != layouts.constEnd(); i++)
    {
        kbLayoutsList.append(qMakePair(i.key(), i.value()));
    }

    for (int i = 0; i < kbLayoutsList.count(); i++)
    {
        QStringList kbVariantsName;
        QStringList kbVariantsDescription;

        QMap<QString, QString> variants = kbLayoutsList[i].second.variants;

        for (const auto& key : variants.keys())
        {
            kbVariantsName << key;
            kbVariantsDescription << variants[key];
        }

        QVariantMap kbLayout;

        kbLayout["name"] = kbLayoutsList[i].first;
        kbLayout["description"] = kbLayoutsList[i].second.description;
        kbLayout["variantsName"] = kbVariantsName;
        kbLayout["variantsDescription"] = kbVariantsDescription;

        kbLayouts.append(kbLayout);
    }
